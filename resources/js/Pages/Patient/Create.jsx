...
export default function CreatePatient(props) {
    // 1. Initialize the Laravel Inertia Form with the Arabic terms from your photo
    const { data, setData, post, processing, errors } = useForm({
        ...
        pharmacist_name: '',
        clinic_code: '',
        // These match the dynamic grids in your terminology drawing
        tests: [{ test_code: '', test_name: '', test_result: '' }],
        medications: [{ med_code: '', med_name: '', dosage: '' }],
    });

    // 2. Logic to add/remove rows for the "Investigations" and "Medications" grids
    const addRow = (type) => {
        const newItem = type === 'tests' 
            ? { test_code: '', test_name: '', test_result: '' }
            : { med_code: '', med_name: '', dosage: '' };
        setData(type, [...data[type], newItem]);
    };
    ...
    const submit = (e) => {
        e.preventDefault();
        // This will connect to the Laravel Controller we will write next
        post(route('patient.store'));
    };

    return (
        <AuthenticatedLayout user={props.auth.user}>
            <Head title="نموذج بيانات المريض" />
            ...
            <div className="py-12 bg-gray-100 min-h-screen">
                ...
                >
                    ...
                    {/* Section 2: التشخيص الطبي (Medical Diagnosis) - The centered box */}
                    <div className="p-6 border border-gray-300 rounded-lg bg-gray-50">
                    ...
                    {/* Section 3: الفحوصات والتحاليل (Labs & Investigations) - The handwritten grid */}
                    <div className="p-6 border border-gray-300 rounded-lg bg-gray-50">
                    ...
                    {/* --- End of Medications Section --- */}





                    {/* Section 4: Footer - Doctor and Pharmacist Signatures */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 border-t-2 border-cyan-600 mt-10">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setData("pharmacist_name", e.target.value)
                                }
                            />
                        </div>
                    </div>
